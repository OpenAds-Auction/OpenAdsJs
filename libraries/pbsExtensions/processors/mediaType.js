...
/**
 * Sets bidResponse.mediaType, using ORTB 2.6 `seatbid.bid[].mtype`, falling back to `ext.openads.type`, falling back to 'banner'.
 */
export function extPrebidMediaType(bidResponse, bid, context) {
  let mediaType = context.mediaType;
  if (!mediaType) {
    mediaType = ORTB_MTYPES.hasOwnProperty(bid.mtype) ? ORTB_MTYPES[bid.mtype] : bid.ext?.openads?.type
